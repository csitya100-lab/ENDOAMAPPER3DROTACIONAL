  ...
  useEffect(() => {
    ...
    loader.load('/model.glb', (gltf) => {
      ...
      report.lesions.forEach(lesion => {
        ...
        marker.position.set(lesion.position.x * 0.5, lesion.position.y * 0.5, lesion.position.z * 0.5);
        scene.add(marker);
      });
    });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-slate-600">Carregando relatório...</div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-8">
            <div>
              ...
              <p className="text-lg font-semibold text-slate-900" data-testid="text-patient-name">{report.patientName}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wide">Data do Exame</p>
              <p className="text-lg font-semibold text-slate-900" data-testid="text-exam-date">{report.examDate}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wide">ID</p>
              <p className="text-lg font-semibold text-slate-900" data-testid="text-patient-id">{report.patientId}</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-3">
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100">
                <h2 className="text-sm font-semibold text-slate-700">Modelo 3D</h2>
                <p className="text-xs text-slate-500">Arraste para rotacionar, scroll para zoom</p>
              </div>
              <div ref={containerRef} className="aspect-square lg:aspect-[4/3] w-full bg-slate-50">
                <canvas ref={canvasRef} className="w-full h-full" data-testid="canvas-3d-model" />
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100">
                <h2 className="text-sm font-semibold text-slate-700">Vistas 2D</h2>
              </div>
              <div className="p-4">
                <div className="grid grid-cols-3 gap-3">
                  {(['sagittal', 'coronal', 'posterior'] as const).map((view) => (
                    <div key={view} className="aspect-square bg-slate-100 rounded-lg overflow-hidden relative group">
                      {report.images2D[view] ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <span className="text-xs text-slate-400 capitalize">{view}</span>
                        </div>
                      )}
                      <div className="absolute bottom-0 left-0 right-0 bg-black/50 py-1 px-2">
                        <span className="text-[10px] text-white capitalize">{view}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="p-4">
                {report.examPhotos.length > 0 ? (
                  ...
                  <div className="aspect-video bg-slate-100 rounded-lg flex items-center justify-center">
                    <span className="text-sm text-slate-400">Nenhuma foto disponível</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mt-6 bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100">
            <h2 className="text-sm font-semibold text-slate-700">Resumo das Lesões</h2>
          </div>
          <div className="p-4">
            {report.lesions.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${severity.text}`}>{lesion.name}</p>
                        <p className="text-xs text-slate-600 truncate">{lesion.location}</p>
                      </div>
                      <span className={`text-xs font-medium ${severity.text} px-2 py-0.5 rounded-full ${severity.bg}`}>
                        {severity.label}
                      </span>
                    </div>
                  );
              ...
              <p className="text-sm text-slate-500 text-center py-4">Nenhuma lesão registrada</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-center gap-2">
          <div className="flex items-center gap-4 text-xs text-slate-500">
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: SEVERITY_COLORS.superficial.hex }} />
              <span>Superficial</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: SEVERITY_COLORS.moderate.hex }} />
              <span>Moderada</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: SEVERITY_COLORS.deep.hex }} />
              <span>Profunda</span>
            </div>
          </div>
        </div>
